...
static inline void set_scan_bm(unsigned long bit, struct single_scan_bm *scan_bm) {
    bitmap_set_bit(bit, scan_bm->bitmap);
...
static int nova_failure_insert_inodetree(struct super_block *sb, unsigned long ino_low,
    ...
    if (ino_low > ino_high) {
        r_error("%s: ino low %lu, ino high %lu", __func__, ino_low, ino_high);
        BUG();
    }
    ...
    if (ret) {
        r_error("%s: ino %lu - %lu already exists!: %d", __func__, ino_low, ino_high, ret);
...
static int nova_init_blockmap_from_inode(struct super_block *sb) {
    ...
    curr_p = pi->log_head;
    if (curr_p == 0) {
        r_error("%s: pi head is 0!", __func__);
        return -EINVAL;
    }
    ...
    while (curr_p != pi->log_tail) {
        ...
        if (curr_p == 0) {
            r_fatal("%s: curr_p is NULL!", __func__);
            ret = -EINVAL;
            break;
        }
...
static int nova_init_inode_list_from_inode(struct super_block *sb) {
    ...
    curr_p = pi->log_head;
    if (curr_p == 0) {
        rd_info("%s: pi head is 0!", __func__);
        return -EINVAL;
    }
    ...
    while (curr_p != pi->log_tail) {
        ...
        if (curr_p == 0) {
            rd_info("%s: curr_p is NULL!", __func__);
            log_assert(0);
        }

        entry = (struct nova_range_node_lowhigh *)nova_get_block(sb, curr_p);
        range_node = nova_alloc_inode_node(sb);
        if (range_node == NULL) log_assert(0);

        cpuid = (entry->range_low & CPUID_MASK) >> 56;
        if (cpuid >= sbi->cpus) {
            r_error("Invalid cpuid %lu", cpuid);
        ...
        if (ret) {
            r_error("%s failed, %d", __func__, cpuid);
            nova_free_inode_node(sb, range_node);
            log_assert(0);
            nova_destroy_inode_trees(sb);
            goto out;
...
static int nova_insert_blocknode_map(struct super_block *sb, int cpuid, unsigned long low,
    ...
    rd_info("%s: cpu %d, low %lu, high %lu, num %lu", __func__, cpuid, low, high, num_blocks);
    free_list = nova_get_free_list(sb, cpuid);
...
static int __nova_build_blocknode_map(struct super_block *sb, unsigned long *bitmap,
    ...
    while (1) {
        ...
        if (nova_insert_blocknode_map(sb, cpuid, low << scale, (next << scale) - 1)) {
            r_error("Error: could not insert %lu - %lu", low << scale, ((next << scale) - 1));
        }
        start = next;
        if (next == bsize) break;
        if (next == end) {
...
// 最后一个用于主线程
struct scan_bitmap *global_bm[65];
...
static int nova_build_blocknode_map(struct super_block *sb, unsigned long initsize) {
    ...
    final_bm = (scan_bitmap*)ZALLOC(sizeof(struct scan_bitmap));
    ...
    final_bm->scan_bm_4K.bitmap = (unsigned long *)ZALLOC(final_bm->scan_bm_4K.bitmap_size);
    ...
    FREE(final_bm->scan_bm_4K.bitmap);
    FREE(final_bm);
...
static void free_bm(struct super_block *sb) {
    ...
    for (i = 0; i <= sbi->cpus; i++) {
        bm = global_bm[i];
        if (bm) {
            FREE(bm->scan_bm_4K.bitmap);
            FREE(bm->scan_bm_2M.bitmap);
            FREE(bm->scan_bm_1G.bitmap);
            FREE(bm);
        }
    }
}
...
static int alloc_bm(struct super_block *sb, unsigned long initsize) {
    ...
    for (i = 0; i <= sbi->cpus; i++) {
        bm = (scan_bitmap*)ZALLOC(sizeof(struct scan_bitmap));
        if (!bm) return -ENOMEM;
        ...
        /* Alloc memory to hold the block alloc bitmap */
        bm->scan_bm_4K.bitmap = (unsigned long *)ZALLOC(bm->scan_bm_4K.bitmap_size);
        bm->scan_bm_2M.bitmap = (unsigned long *)ZALLOC(bm->scan_bm_2M.bitmap_size);
        bm->scan_bm_1G.bitmap = (unsigned long *)ZALLOC(bm->scan_bm_1G.bitmap_size);
...
struct task_ring {
    ...
    int cpuid;
    super_block *sb;
    u64 addr[512]; // 存放要扫描的inode table
    int num;
    int inodes_used_count;
    u64 *array;  // 存放文件某个偏移范围的block有效情况
};
...
static int nova_traverse_dir_inode_log(struct super_block *sb, struct nova_inode *pi,
    ...
    curr_p = pi->log_head;
    if (curr_p == 0) {
        r_error("Dir %lu log is NULL!", pi->nova_ino);
        BUG();
    }
    ...
    while (curr_p != pi->log_tail) {
        ...
        switch (type) {
            ...
            case FILE_WRITE:
                break;
            default:
                rd_info("%s: unknown type %d, 0x%lx", __func__, type, curr_p);
                log_assert(0);
        }
...
static int nova_recover_inode_pages(struct super_block *sb, struct nova_inode_info_header *sih,
    ...
    rd_info("%s: inode %lu, addr 0x%lx, head 0x%lx, tail 0x%lx", __func__, nova_ino, pi_addr,
              pi->log_head, pi->log_tail);
...
static void* failure_thread_func(void *data);

static int allocate_resources(struct super_block *sb, int cpus) {
    struct task_ring *ring;
    int i;

    task_rings = (task_ring*)ZALLOC((cpus+1) * sizeof(struct task_ring));
    if (!task_rings) goto fail;

    for (i = 0; i <= cpus; i++) {
        ring = &task_rings[i];
        ring->total_cpus = cpus;
        ring->cpuid = i;
        ring->sb = sb;
        ring->array = (u64*)ZALLOC(sizeof(u64) * MAX_PGOFF);
        if (!ring->array) goto fail;
    }

    // threads = ZALLOC(cpus * sizeof(struct task_struct *));
    ...
    //     int ret = pthread_create(&threads[i], nullptr, failure_thread_func, &task_rings[i]);
    //     log_assert(ret == 0);
    // }
...
static void* failure_thread_func(void *data) {
    task_ring* ring = (task_ring*)data;
    struct super_block *sb = ring->sb;
    ...
    for (count = 0; count < ring->num; count++) {
        ...
        for (i = 0; i < num_inodes_per_page; i++) {
            ...
            pi = (nova_inode*)nova_get_block(sb, pi_addr);
            if (pi->valid) {
...
static int nova_failure_recovery_crawl(struct super_block *sb) {
    ...
    int ring_id;
    int inode_tables = 0;
    // int thread_num = sbi->cpus;
    int thread_num = 4;

    ring_id = 0;
    for (cpuid = 0; cpuid < thread_num; cpuid++) {
        ...
        while (curr) {
            ...
            if (ring->num >= 512) {
                r_error("%s: ring size too small", __func__);
                return -EINVAL;
            }
    ...
    rd_info("%s log_tables %lu to scan.", __func__, inode_tables);

    for (cpuid = 0; cpuid < thread_num; cpuid++) {
        // threads[i] = kthread_create(failure_thread_func, sb, "recovery thread");
        // kthread_bind(threads[i], i);
        int ret = pthread_create(&threads[cpuid], nullptr, failure_thread_func, &task_rings[cpuid]);
        log_assert(ret == 0);
    }
    ...
    nova_recover_inode_pages(sb, &sih, &task_rings[sbi->cpus], root_addr, global_bm[sbi->cpus]);

    wait_to_finish(thread_num);
...
int nova_failure_recovery(struct super_block *sb) {
    ...
    rd_info("Failure recovery total recovered %lu", sbi->s_inodes_used_count);
    return ret;
}
