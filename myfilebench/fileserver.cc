...
void* fileserver_job(void* arg) {
    ...
    printf("thread %d: cpu_id=%d, dir=%s, op_num=%lu, nfiles=%d\n",
    ...
    for(uint64_t i = 0; i < job_arg->op_num; ++i) {
        file_idx = i % t_nfiles;
        int write_file_i;
        dir_path = GetDirPath(dir, depth, file_idx, &write_file_i);
        int read_file_i = (write_file_i + 1) % DIR_WIDTH;
        ...
        log_assert(fd > 0);
        // append write
        append_size = GetRandSize(meanappendsize, 12);
