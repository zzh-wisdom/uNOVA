...
struct _tickets { __tickets head, tail; };
struct ticket_lock_t {
    union {
        __ticketpair ticket_pair;
        struct _tickets tickets;
    };
};

static __inline__ void ticket_lock_init(struct ticket_lock_t *lock)
{
    *lock = (struct ticket_lock_t)__SPIN_LOCK_INITIALIZER;
}
 ...
 */
static __inline__ void ticket_lock(struct ticket_lock_t *lock)
{
...
#ifdef DEBUG
    printf("Thread [%llu] got the lock\n", (unsigned long long)pthread_self());
#endif
 ...
 */
static __inline__ void ticket_unlock(struct ticket_lock_t *lock)
{
...
static __inline__ int ticket_trylock(struct ticket_lock_t *lock)
{
    struct ticket_lock_t __old, __new;
    __old.ticket_pair  = __ACCESS_ONCE(lock->ticket_pair);
...
typedef ticket_lock_t ticket_semaphore_t;

static __inline__ void ticket_semaphore_init(struct ticket_lock_t *lock)
{
    *lock = (struct ticket_lock_t)__SPIN_LOCK_INITIALIZER;
}
