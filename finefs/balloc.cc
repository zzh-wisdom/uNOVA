...
// 完成整个NVM空间的每个cpu划分
void finefs_init_blockmap(struct super_block *sb, double log_heap_occupy, int recovery) {
    struct finefs_sb_info *sbi = FINEFS_SB(sb);
    ...
    unsigned long per_2_list_blocks;
    unsigned long per_log_list_blocks;
    ...
    per_2_list_blocks = sbi->num_blocks / sbi->cpus;
    log_assert(per_2_list_blocks > 2);
    per_log_list_blocks = per_2_list_blocks * log_heap_occupy;
    if (per_log_list_blocks == 0) {
        r_warning("log_heap_occupy %./2lf too low, per_log_list_blocks set to 1", log_heap_occupy);
        per_log_list_blocks = 1;
    }
    per_data_list_blocks = per_2_list_blocks - per_log_list_blocks;
    r_info("per_log_list_blocks=%lu %lfMB, per_data_list_blocks=%lu, %lfMB", per_log_list_blocks,
           per_log_list_blocks * 4.0 / 1024, per_data_list_blocks,
           per_data_list_blocks * 4.0 / 1024);
    sbi->per_2_list_blocks = per_2_list_blocks;
    sbi->per_log_list_blocks = per_log_list_blocks;
    for (i = 0; i < sbi->cpus; i++) {
        ...
        log_free_list->block_start = per_2_list_blocks * i;
        log_free_list->block_end = log_free_list->block_start + per_log_list_blocks - 1;

        data_free_list->block_start = log_free_list->block_end + 1;
        data_free_list->block_end = data_free_list->block_start + per_data_list_blocks - 1;

        /* For recovery, update these fields later */
        if (recovery == 0) {  // 此时是没有恢复的情况
            log_free_list->num_free_blocks = per_log_list_blocks;
            if (i == 0) {  // 第一个要减去预留的block个数
                log_free_list->block_start += num_used_block;
                log_assert(log_free_list->num_free_blocks > num_used_block);
                log_free_list->num_free_blocks -= num_used_block;
            }
            data_free_list->num_free_blocks = per_data_list_blocks;
    ...
    // log_free_list = finefs_get_log_free_list(sb, (sbi->cpus - 1));
    ...
    if (data_free_list->block_end + 1 < sbi->num_blocks) {
        ...
        int total_blk_num = sbi->num_blocks - data_free_list->block_end - 1;
        int log_blk_num = total_blk_num * log_heap_occupy;
        log_assert(log_blk_num);
        log_assert(total_blk_num - log_blk_num > 0);
        sbi->shared_log_free_list.block_start = data_free_list->block_end + 1;
        sbi->shared_log_free_list.block_end =
            sbi->shared_log_free_list.block_start + log_blk_num - 1;
        sbi->shared_data_free_list.block_start = sbi->shared_log_free_list.block_end + 1;
        sbi->shared_data_free_list.block_end = sbi->num_blocks - 1;
    }
}

static inline int finefs_rbtree_compare_rangenode(struct finefs_range_node *curr,
                                                  unsigned long range_low) {
    if (range_low < curr->range_low) return -1;
    if (range_low > curr->range_high) return 1;

    return 0;
}

static int finefs_find_range_node(struct finefs_sb_info *sbi, struct rb_root *tree,
                                  unsigned long range_low, struct finefs_range_node **ret_node) {
...
static int finefs_insert_range_node(struct finefs_sb_info *sbi, struct rb_root *tree,
    ...
    while (*temp) {
        curr = container_of(*temp, struct finefs_range_node, node);
        compVal = finefs_rbtree_compare_rangenode(curr, new_node->range_low);
        parent = *temp;
        ...
        if (compVal == -1) {
            ...
            r_error(
                "%s: entry %lu - %lu already exists: "
                "%lu - %lu",
                __func__, new_node->range_low, new_node->range_high, curr->range_low,
                curr->range_high);
            return -EINVAL;
        }
    }
...
inline int finefs_insert_blocktree(struct finefs_sb_info *sbi, struct rb_root *tree,
    ...
    if (ret) r_error("ERROR: %s failed %d", __func__, ret);
...
int finefs_insert_inodetree(struct finefs_sb_info *sbi, struct finefs_range_node *new_node,
    ...
    tree = &sbi->inode_maps[cpu].inode_inuse_tree;
    ret = finefs_insert_range_node(sbi, tree, new_node);
    if (ret) rd_error("ERROR: %s failed %d", __func__, ret);
...
int finefs_find_free_slot(struct finefs_sb_info *sbi, struct rb_root *tree, unsigned long range_low,
    ...
    if (ret) {
        rd_error("%s ERROR: %lu - %lu already in free list", __func__, range_low, range_high);
        return -EINVAL;
    }
    ...
    if (!ret_node) {
        ...
        *next = ret_node;
        ...
        rd_error(
            "%s ERROR: %lu - %lu overlaps with existing node "
            "%lu - %lu",
            __func__, range_low, range_high, ret_node->range_low, ret_node->range_high);
        return -EINVAL;
    }
...
static int finefs_free_blocks(struct super_block *sb, unsigned long blocknr, int num,
                              unsigned short btype, int log_page) {
    struct finefs_sb_info *sbi = FINEFS_SB(sb);
    struct rb_root *tree;
    ...
    cpuid = get_cpuid(sbi, blocknr);
    is_log_heap = (blocknr % sbi->per_2_list_blocks) < sbi->per_log_list_blocks;
    ...
    rdv_proc("Free %s block to %s heap: %lu - %lu", is_log_heap ? "[log]" : "[data]",
             log_page ? "[log]" : "[data]", block_low, block_high);

    ret = finefs_find_free_slot(sbi, tree, block_low, block_high, &prev, &next);

    if (ret) {
        r_fatal("%s: find free slot fail: %d", __func__, ret);
    ...
    if (!prev) free_list->first_node = curr_node;
    free_list->num_blocknode++;
...
out:
    spin_unlock(&free_list->s_lock);
    if (new_node_used == 0) finefs_free_blocknode(sb, curr_node);

    return ret;
}

int finefs_free_data_blocks(struct super_block *sb, struct finefs_inode *pi, unsigned long blocknr,
                            int num) {
    int ret;
    timing_t free_time;
    struct finefs_sb_info *sbi = FINEFS_SB(sb);

    rd_info("Inode %lu: free %d data block from %lu to %lu", pi->finefs_ino, num, blocknr,
            blocknr + num - 1);
    if (blocknr == 0) {
    ...
    // data page 释放到 log heap中需要清零
    bool is_log_heap = (blocknr % sbi->per_2_list_blocks) < sbi->per_log_list_blocks;
    void *bp = finefs_get_block(sb, finefs_get_block_off(sb, blocknr, pi->i_blk_type));
    int num_blocks = finefs_get_numblocks(pi->i_blk_type) * num;
    memset_nt(bp, 0, FINEFS_BLOCK_SIZE * num_blocks);
...
int finefs_free_log_blocks(struct super_block *sb, struct finefs_inode *pi, unsigned long blocknr,
                           int num) {
    int ret;
    timing_t free_time;

    rd_info("Inode %lu: free %d log block from %lu to %lu", pi->finefs_ino, num, blocknr,
            blocknr + num - 1);
    if (blocknr == 0) {
        r_error("%s: ERROR: %lu, %d", __func__, blocknr, num);
        return -EINVAL;
    ...
    while (temp) {
        ...
        if (num_blocks >= curr_blocks) {
            ...
            if (curr == free_list->first_node) {
                ...
                if (next_node) next = container_of(next_node, struct finefs_range_node, node);
...
static int finefs_new_blocks_from_data_heap(struct super_block *sb, unsigned long *blocknr,
    ...
    rd_info("[From Data Heap] new block size: %lu, type %s", num_blocks << FINEFS_BLOCK_SHIFT,
            atype == LOG ? "LOG" : "DATA");
    if (num_blocks == 0) return -EINVAL;
...
retry:
    ...
    if (free_list->num_free_blocks < num_blocks || !free_list->first_node) {
        rd_info(
            "%s: cpu %d, free_blocks %lu, required %lu, "
            "blocknode %lu",
            __func__, cpuid, free_list->num_free_blocks, num_blocks, free_list->num_blocknode);
        if (free_list->num_free_blocks >= num_blocks) {
            ...
            spin_unlock(&free_list->s_lock);
            if (retried >= ALLOC_BLOCK_RETRY) return -ENOSPC;
            // 从其他cpu分配空闲空间
    ...
    ret_blocks = finefs_alloc_blocks_in_free_list(sb, free_list, btype, num_blocks, &new_blocknr);
    ...
    if (ret_blocks <= 0 || new_blocknr == 0) return -ENOSPC;

    if (zero) {
        bp = finefs_get_block(sb, finefs_get_block_off(sb, new_blocknr, btype));
...
static int finefs_new_blocks_from_log_heap(struct super_block *sb, unsigned long *blocknr,
                                           unsigned int num, unsigned short btype, int zero,
                                           enum alloc_type atype, int cpuid) {
    struct free_list *free_list;
    void *bp;
    ...
    num_blocks = num * finefs_get_numblocks(btype);
    rd_info("[From Log Heap] new block size: %lu, type %s", num_blocks << FINEFS_BLOCK_SHIFT,
            atype == LOG ? "LOG" : "DATA");
    if (num_blocks == 0) return -EINVAL;
...
static int finefs_new_blocks(struct super_block *sb, unsigned long *blocknr, unsigned int num,
                             unsigned short btype, int zero, enum alloc_type atype,
                             int cpuid = -1) {
    int ret;
    if (cpuid == -1) cpuid = get_processor_id();
    if (atype == LOG) {
        ret = finefs_new_blocks_from_log_heap(sb, blocknr, num, btype, zero, atype, cpuid);
        if (ret > 0) return ret;
        dlog_assert(ret == -ENOSPC);
        r_info("cpuid [%d] Log heap full, alloc log page from data heap", cpuid);
        return finefs_new_blocks_from_data_heap(sb, blocknr, num, btype, 1, atype, cpuid);
    } else {
        ret = finefs_new_blocks_from_data_heap(sb, blocknr, num, btype, zero, atype, cpuid);
        if (ret > 0) return ret;
        dlog_assert(ret == -ENOSPC);
        r_info("cpuid [%d] Data heap full, alloc data page from log heap", cpuid);
        return finefs_new_blocks_from_log_heap(sb, blocknr, num, btype, zero, atype, cpuid);
    }
}

// 分配文件数据块
int finefs_new_data_blocks(struct super_block *sb, struct finefs_inode *pi, unsigned long *blocknr,
                           unsigned int num, unsigned long start_blk, int zero, int cow) {
    int allocated;
    timing_t alloc_time;
    FINEFS_START_TIMING(new_data_blocks_t, alloc_time);
    // allocated = finefs_new_blocks(sb, blocknr, num,
    // 				pi->i_blk_type, zero, DATA);
    allocated = finefs_new_blocks(sb, blocknr, num, FINEFS_DEFAULT_DATA_BLOCK_TYPE, zero, DATA);
    FINEFS_END_TIMING(new_data_blocks_t, alloc_time);
    rdv_proc(
        "Inode %lu, start blk %lu, cow %d, "
        "alloc %d data blocks from %lu to %lu",
        pi->finefs_ino, start_blk, cow, allocated, *blocknr, *blocknr + allocated - 1);
    return allocated;
}
...
// zero为1表示新分配的空间需要清零
int finefs_new_log_blocks(struct super_block *sb, struct finefs_inode *pi, unsigned long *blocknr,
                          unsigned int num, int zero, int cpuid) {
    int allocated;
    timing_t alloc_time;
    FINEFS_START_TIMING(new_log_blocks_t, alloc_time);
    allocated = finefs_new_blocks(sb, blocknr, num, pi->i_blk_type, zero, LOG, cpuid);
    FINEFS_END_TIMING(new_log_blocks_t, alloc_time);
    rdv_proc("Inode %lu, alloc %d log blocks from %lu to %lu", pi->finefs_ino, allocated, *blocknr,
             *blocknr + allocated - 1);
    return allocated;
}
